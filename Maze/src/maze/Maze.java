...
public class Maze extends JPanel {
    ...
    public Maze(int rows, int columns) {
        ...
        makeMazePretty();
        getRandomEmptyCell().setType(MazeCell.CellType.TRAIL_HEAD);
        getRandomEmptyCell().setType(MazeCell.CellType.EXIT);
    } // end constructor
    ...
    public MazeCell getTrailHead() {
        for (int row = 0; row < rows; row++) {
            for (int column = 0; column < columns; column++) {
                MazeCell cell = maze[row][column];
                if (cell.getType() == MazeCell.CellType.TRAIL_HEAD) {
    ...
    private void fillMaze(int minRow, int minCol, int maxRow, int maxCol) {
        ...
        for (int row = minRow; row <= maxRow; row++) {
            maze[row][wallCol].setType(MazeCell.CellType.WALL);
        }
        for (int col = minCol; col <= maxCol; col++) {
            maze[wallRow][col].setType(MazeCell.CellType.WALL);
        }
        ...
        maze[northPosition][wallCol].setType(MazeCell.CellType.EMPTY);
        maze[southPosition][wallCol].setType(MazeCell.CellType.EMPTY);
        maze[wallRow][eastPosition].setType(MazeCell.CellType.EMPTY);
        maze[wallRow][westPosition].setType(MazeCell.CellType.EMPTY);
    ...
    /**
     * makeMazePretty:  This method reduces 2x2 grids of empty cells.  While
     * not necessary, it tends to make a more pleasing looking maze.
     */
    private void makeMazePretty() {
        for (int row = 0; row < rows; row = row + 1) {
            for (int column = 0; column < columns; column = column + 1) {
                // test if upper left corner of a 2x2 rectangle of empty space
                if ((getCellType(row,column) != MazeCell.CellType.EMPTY) ||
                    (getCellType(row+1,column) != MazeCell.CellType.EMPTY) ||
                    (getCellType(row,column+1) != MazeCell.CellType.EMPTY) ||
                    (getCellType(row+1,column+1) != MazeCell.CellType.EMPTY) ){
                     continue;
                }
                maze[row][column].setType(MazeCell.CellType.WALL);
            }
        }
        
    } // end makeMazePretty 

    private MazeCell.CellType getCellType(int row, int column) {
        if (row < 0) return MazeCell.CellType.WALL;
        if (column < 0) return MazeCell.CellType.WALL;
        if (row > rows - 1) return MazeCell.CellType.WALL;
        if (column > columns - 1) return MazeCell.CellType.WALL;
        return maze[row][column].getType();
    } // end method getCell
    ...
    private MazeCell getRandomEmptyCell() {
        ...
        do {
            ...
            cell = maze[row][col];
        } while (cell.getType() != MazeCell.CellType.EMPTY);
        return cell;
